...
int totalNumberOfGenes = 10;
// Total ribosomes.  Command line option provided.
int totalNumberOfRibosomes = 7.3e4;
// Fraction of ribosomes bound to mRNA (rich)
double boundRibosomeFraction = 0.85;
// Fraction of ribosomes bound to mRNA (starved)
double boundRibosomeFractionStarvation = 0.95;
// Total tRNAs
int totalNumberOfTrna = 1; //6.8e5; is the actual number; but this is already in concn.
...
// tRNA concentrations.
// Set from command line.
char trnaConcentrationFile[250] = "";
...
// This file and data structure holds the fit data for stall strengths at specific pause codons
// in YFP                                    
char stallStrengthFitFile[150] = "";
map <int, map <int,double> > stallStrengthArray;
bool useStallStrengthFile = false;

// Threshold time for starting analysis.
// Command line option provided.
double thresholdTime = 10;
// Variables for reporter-only simulation.
//
// The number of proteins from this mrna will be tracked
int mrnaOfInterest = 0;
// Variables are tracked after these many proteins are
// produced from the mrna of interest
int thresholdNumberOfProteins = 100;
// Variables are tracked until these many proteins are
// produced from the mrna of interest
int requiredNumberOfProteins = 10000;
...
int Read_sequence_File(char *filename, gene *Genes, int &totalNumberOfGenes) {
   FILE *fh;
  int c1=0,c2=0;
  char curr_char;
  char buffer[5000];
  ...
  // Update the total number of genes.
  totalNumberOfGenes = c1;
}

// Read in stall strength file 
int Read_Fit_data(char *stallStrengthFitFile, map <int, map <int,double> > &stallStrengthArray) {

  map<int,map<int,double> >::iterator outerit;
  map<int,double>::iterator innerit;
  string line;
  int codon[300] = {0};
  int pos[300];
  double stallStrength[300];
  ifstream fin(stallStrengthFitFile);
  int count = 0;

  if (fin.is_open()) {
    // read header
    getline(fin, line);
    while (fin >> codon[count] >> pos[count] >> stallStrength[count]) {
      count++;
    }
  } else {
  ...
  for (count=0;count<300;count++) {
    if (codon[count] != 0) {
      stallStrengthArray[codon[count]][pos[count]] = stallStrength[count];
    }
  }
  return 0;
}
...
void read_trna_filename(int argc, char *argv[]) {
  int argIndex;
  for (argIndex=1;argIndex<argc;argIndex++) {
    if ( !strcmp(argv[argIndex], "--trna-concn") ) {
      strcat( trnaConcentrationFile, argv[argIndex+1] );
    }
  }
}
...
void Read_Commandline_Args(int argc, char *argv[], trna *trna_list) {
  ...
  for (argIndex=1;argIndex<argc;argIndex++) {
    ...
    if ( !strcmp(argv[argIndex], "--stall-strength-file") ) {
      strcat(stallStrengthFitFile, argv[argIndex+1]);
      useStallStrengthFile = true;
      strcat( outfilePrefix, "_stallstrengthfile_" );
      string s(argv[argIndex+1]);
      boost::smatch m;
      boost::regex e("/([^/]*).tsv");
...
int TerminateRibosome(
  ...
  // Track the number of full and aborted proteins right from the start
  if (ribosome_list[r_id].pos >= Genes[mRNA[m_id].gene].length - 1) {
...
// READ INPUT FILES AND PARAMETERS 
  ...
  Read_sequence_File(mrnaFile, Genes, totalNumberOfGenes);

  // Read in experimental data fits
  if (useStallStrengthFile == true) {
    Read_Fit_data(stallStrengthFitFile, stallStrengthArray);
  }
...
///////////////////////////////////////////////////////////////////////////////
  ...
  // Total single tRNA accommodation rate, does not change for reporter simulations
    for (codonloop=0;codonloop<61;codonloop++) {
      singleTrnaAccommodationRate[codonloop] = 0.0;
      for (cognatetrnaloop=0; cognatetrnaloop<2; cognatetrnaloop++) {
        int trna_id;
        trna_id = Codon[codonloop].trna[cognatetrnaloop];
        singleTrnaAccommodationRate[codonloop] += ( trna_list[trna_id].trnaRibosomeRateConstant * trna_list[trna_id].concn * Codon[codonloop].weight[cognatetrnaloop] );
      }
      //cout << "Codon" << codonloop << ", " << singleTrnaAccommodationRate[codonloop] << "s-1\n";
    }
...
// (tRNA concentrations, mRNA states, lists of ribosomes in different states)
  ...
  // Run simulation till these many proteins are produced from the mrna of interest
  while (mRNA[mrnaOfInterest].numberOfFullProteins < requiredNumberOfProteins) {
    if ((start_tracking == false) && (mRNA[mrnaOfInterest].numberOfFullProteins >= thresholdNumberOfProteins)) {
    ...
    totalInitiationRate = totalInitiationScaledMrnaCopyNumber * boundRibosomeFractionStarvation / boundRibosomeFraction;
    ...
    for (codonloop=0;codonloop<61;codonloop++) {
//
//      // Iterate over every ribosome that has A-site empty with this codon.
//      // This iteration is done only with reporters to account for site-specific
//      // experimentally measured rates.
//      for (pos_in_aempty_list=0;pos_in_aempty_list<n_Rb_ae[codonloop]; pos_in_aempty_list++) {
//        r_id = Rb_ae[codonloop][pos_in_aempty_list];
//        // The mRNA is whichever mRNA the picked ribosome was bound to.
//        m_id = ribosome_list[r_id].mRNA;
//        // Position of this ribosome on the mRNA.
//        pos = ribosome_list[r_id].pos;
//        // These are the two reporter mRNAs, 0 is control, 1 is the mutant.
//        if (m_id == 0 or m_id == 1) {
//
      if (stallStrengthArray.find(codonloop) == stallStrengthArray.end()) {
        totalTrnaAccommodationRate += singleTrnaAccommodationRate[codonloop] * n_Rb_ae[codonloop];
      } else {
        for (pos_in_aempty_list=0;pos_in_aempty_list < n_Rb_ae[codonloop];pos_in_aempty_list++) {
          r_id = Rb_ae[codonloop][pos_in_aempty_list];
          // Whichever mRNA the picked ribosome was bound to.
          m_id = ribosome_list[r_id].mRNA;
          // Ribosome position on the mRNA.
          pos = ribosome_list[r_id].pos;
          if (stallStrengthArray[codonloop].find(pos) != stallStrengthArray[codonloop].end()) {
            totalTrnaAccommodationRate += stallStrengthArray[codonloop][pos];
          } else {
            totalTrnaAccommodationRate += singleTrnaAccommodationRate[codonloop];
          }
        }
      }
    }
	...
	if (singleTrnaAccommodationRate[codonloop] < thresholdTrnaAccommodationRate) {
	  ...
	  continue;
        ...
        for (pos_in_aempty_list=0;pos_in_aempty_list < n_Rb_ae[codonloop];pos_in_aempty_list++) {
          ...
          pos = ribosome_list[r_id].pos;
          if (stallStrengthArray[codonloop].find(pos) != stallStrengthArray[codonloop].end()) {
	    if (stallStrengthArray[codonloop][pos] < thresholdTrnaAccommodationRate) {
	      totalSelectivePrematureTerminationRate += selectivePrematureTerminationRate;
	    } else {
	      continue;
	    }
          } else {
...
///////////////////////////////////////////////////////////////////////////////
    ...
    if (randomRate > previousRate && randomRate < currentRate) {
      ...
      for (geneloop=0;geneloop<totalNumberOfGenes;geneloop++) {
        previousRate += initiationScaledMrnaCopyNumber[geneloop] * boundRibosomeFractionStarvation / boundRibosomeFraction;
        if (randomRate < previousRate) {
    ...
    if (randomRate > previousRate && randomRate < currentRate) {
      ...
      for (codonloop=0;codonloop<61;codonloop++) {
        if (stallStrengthArray.find(codonloop) == stallStrengthArray.end()) {
          previousRate += singleTrnaAccommodationRate[codonloop] * n_Rb_ae[codonloop];
          if (randomRate < previousRate) {
            c_id = codonloop;
            //cout << "c_id = " << c_id << ", m_id = " << m_id << ", r_id = " << r_id << "\n";
          ...
          c_id = codonloop;
          for (int loop1=0;loop1 < n_Rb_ae[codonloop];loop1++) {
            r_id = Rb_ae[c_id][loop1];
            // Whichever mRNA the picked ribosome was bound to.
            m_id = ribosome_list[r_id].mRNA;
            // Ribosome position on the mRNA.
            pos = ribosome_list[r_id].pos;
            if (stallStrengthArray[codonloop].find(pos) != stallStrengthArray[codonloop].end()) {
              // Use fit parameter if it is available
              previousRate += stallStrengthArray[codonloop][pos];
            } else {
              // If fit parameter is not available, use default value
              previousRate += singleTrnaAccommodationRate[codonloop];
            }
            if (randomRate < previousRate) {
              pos_in_aempty_list = loop1;
              //cout << "c_id = " << c_id << ", m_id = " << m_id << ", r_id = " << r_id << ", pos = " << pos << "\n";
              goto brokenloop;
            }
          }
        }
    ...
    brokenloop:
      ...
      for (cognatetrnaloop=0;cognatetrnaloop<2;cognatetrnaloop++) {
        int trna_id;
        trna_id = Codon[c_id].trna[cognatetrnaloop];
        prob_g += ( trna_list[trna_id].trnaRibosomeRateConstant
            * trna_list[trna_id].concn
    ...
    if (randomRate > previousRate && randomRate < currentRate) {
      for (codonloop=0;codonloop<61;codonloop++) {
        if (stallStrengthArray.find(codonloop) == stallStrengthArray.end()) {
	  if (singleTrnaAccommodationRate[codonloop] < thresholdTrnaAccommodationRate) {
	    previousRate += selectivePrematureTerminationRate * n_Rb_ae[codonloop];
	    if (randomRate < previousRate) {
	      c_id = codonloop;
	      // Randomly pick an A-site empty ribosome bound to codon c_id
	      pos_in_aempty_list = rand()%n_Rb_ae[c_id];
    ...
    if (
      ...
      tracking_n_Rb_hit5 = n_Rb_hit5;
      tracking_n_Rb_hit3 = n_Rb_hit3;
      trackingISMCN = mRNA[mrnaOfInterest].numberOfFullProteins;
    }
    numberOfIterations++;
